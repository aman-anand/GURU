...
class OptionalHeader extends React.PureComponent {
  // eslint-disable-line react/prefer-stateless-function
  changeAnimateFn = goTo => {
    const { changeAnimate } = this.props;
    if (changeAnimate) {
      changeAnimate(goTo);
    }
    return null;
  ...
  render() {
    const { goTo, title, sendCloseMessage, isMobile } = this.props;
    return (
      <Fragment>
        {isMobile ? (
          <OptionalHeaderContainer>
            <div className="backTo" onClick={() => this.changeAnimateFn(goTo)} role="presentation">
              <Link to={goTo} onClick={() => sendCloseMessage({ screenUrl: '/sales' })}>
                <span className="">
...
OptionalHeader.propTypes = {
  ...
  changeAnimate: PropTypes.func,
  sendCloseMessage: PropTypes.func,
  isMobile: PropTypes.bool,
};

export default withSizes(mapSizesToProps)(OptionalHeader);
